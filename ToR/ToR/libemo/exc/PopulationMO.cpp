...
namespace az
{
	namespace mea
	{
		...
		// erase the last individual
		void CPopulationMO::EraseOne()
		{
			if (mvPop[Size() - 1] > 0) delete mvPop[Size() - 1];
		}

		void CPopulationMO::Pop(unsigned int i)
		{
			mvPop.erase(mvPop.begin(), mvPop.begin() + i);
		}
		...
		void CPopulationMO::RankSort(bool esort)
		{
			...
			if (esort)
			{
				while (s<t)
				{
					while (s < int(Size()) && In(s).IsEFeasible()) s++;
					while (t >= 0 && !In(t).IsEFeasible()) t--;
					if (s<t)	{ Swap(s, t); s++; t--; }
				}
			}
			else
			...
			//Step 1: Initialize
			for (unsigned int i = 0; i < size; i++) vAssign[i] = false;
			...
			for (unsigned int i = 0; i < size; i++)
			{
				...
				{
					better = esort ? In(i).CEDominate(In(j)) : In(i).CDominate(In(j));
					//i is dominated by j
			...
			while (noAssign > 0)
			{
				...
				for (unsigned int i = 0; i<size; i++) if (!vAssign[i])
				{
					if (vBeDom[i]<minRank)
					{
						minRank = vBeDom[i];
			...
			curRank++; for (unsigned int i = size; i<Size(); i++) In(i).Rank(curRank);
		...
		// assign rank value and sort population without considering constraint
		void CPopulationMO::URankSort()
		{
			//has been sorted before
			if (IsSort()) return;
			...
			//Step 4: Sort the population by rank and constraint
			for (s = 0; s<int(Size() - 1); s++) for (t = s + 1; t<int(Size()); t++) if (In(t)<In(s)) Swap(s, t);
