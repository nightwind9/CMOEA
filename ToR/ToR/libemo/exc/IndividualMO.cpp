...
namespace az
{
	namespace mea
	{
		...
		//e-dominance check
		int CIndividualMO::CEDominate(CIndividualMO& ind)
		{
			if (IsEFeasible() && !ind.IsEFeasible()) { return 1; }
			else if (!IsEFeasible() && ind.IsEFeasible()) { return -1; }
			else if (!IsEFeasible() && !ind.IsEFeasible())
			{
				if (mConstraint < ind.mConstraint) { return 1; }
				else { return -1; }
			}
		...
		void CIndividualMO::Evaluate()
		{
			...
			for (i = 0; i<P().ESize(); i++) 
			{ 
				if (wxFinite(mvEq[i]) == 0) mvEq[i] = 1.0E20; 
				mvEq[i] = (fabs(mvEq[i]) - P().TolC())>0 ? (fabs(mvEq[i]) - P().TolC()) : 0; 
				mConstraint += mvEq[i];
			}
			...
			for (i = 0; i<P().ISize(); i++) 
			{ 
				if (wxFinite(mvIneq[i]) == 0) mvIneq[i] = 1.0E20; 
				mvIneq[i] = mvIneq[i] > 0.0 ? mvIneq[i] : 0.0; 
				mConstraint += mvIneq[i]; 
			}
		}
